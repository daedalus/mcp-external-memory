#!/usr/bin/env python3
"""Install the external-memory skill to Claude Code, OpenCode, and Codex."""
...
def install_skill(agent: str, target_dir: Path) -> bool:
    ...
    try:
        ...
        print(f"Installed {SKILL_NAME} to {agent} at {target_path}")
        return True
    except OSError:
        shutil.copytree(SOURCE_DIR.resolve(), target_path)
        print(f"Copied {SKILL_NAME} to {agent} at {target_path}")
        return True
...
def main() -> int:
    ...
    print("\nInstallation complete! Restart Claude/OpenCode to use the skill.")
    return 0
